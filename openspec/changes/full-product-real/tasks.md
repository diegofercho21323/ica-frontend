...
## Phase F5-PR3: Blind-Safe Telemetry — spec: product-telemetry "Capture and submission metrics" — COMPLETE (F1-F5 all done)

- [x] 6.5 RED `src/shared/lib/telemetry/telemetry.test.ts` (8 cases): source-scan + runtime-shape structural proof of zero quantity/stock/priorCount/variance fields; median/p90 focus-to-save aggregate (never a raw array); per-attempt isolation; independent unit-error/retry counters; non-finite/negative durations rejected without throwing; every `record*` returns `void` (fire-and-forget). Plus 2 new integration assertions in `useGuidedCapture.test.tsx` (focus-to-save sample recorded on save, `unitErrorCount` on unit-mismatch) and 1 new case in `SubmissionQueue.test.tsx` (retry count survives a fresh 409). RED confirmed: module unresolved, then the 2 wiring assertions failed against real `0`/`undefined` before any production edit.
- [x] 6.6 GREEN add `src/shared/lib/telemetry/telemetry.ts` — non-blocking (`safely()` swallow-wrapped, synchronous, `void`-returning) aggregate collector, in-memory `Map<attemptId, state>`, ring-bufferred to 200 samples. Wired additively into `useGuidedCapture.ts` (`focusStartRef` timer started on `selectLine`/initial auto-select/`advanceAfterSave`, recorded in the save mutation's `onSuccess`; `recordUnitError` on the existing `unit-mismatch` field-error branch) and `SubmissionQueue.tsx`'s `handleRetry` (`recordRetry` before the submit call). Neither hook's existing tested behavior changed — telemetry is a pure side effect. `npm run test:run -- --no-file-parallelism` 338/338 (was 329/329, +9); typecheck/lint/fsd clean. commit `feat(telemetry): blind-safe metrics`

**F1–F5 are now all complete.** Every phase task (1.1–6.6) across F1 (Reconcile), F2 (Contract skeleton, gated), F3 (Capture), F4 (Close-loop), F5 (Offline) is `[x]`.
